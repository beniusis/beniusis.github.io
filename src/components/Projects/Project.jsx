export const Project = ({ data }) => {
  return (
    <div className="flex w-full items-center justify-between">
      <a
        className="flex w-full max-w-[80%] cursor-pointer flex-col items-start justify-center text-base text-zinc-900 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:opacity-70 dark:text-zinc-50 xs:text-sm"
        href={data.url}
        target="_blank"
      >
        <h3 className="font-bold">{data.title}</h3>
        <p className="text-pretty font-normal">{data.description}</p>
      </a>

      <a
        className="h-8 w-8 cursor-pointer xs:h-7 xs:w-7"
        href={data.repo}
        target="_blank"
      ...
      >
        <svg
          className="h-full w-full fill-zinc-900 hover:opacity-70 dark:fill-zinc-50"
          viewBox="0 0 24 24"
        >
